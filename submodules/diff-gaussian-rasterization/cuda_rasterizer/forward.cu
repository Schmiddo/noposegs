...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, float* campos, const float* shs, bool* clamped)
{
	// The implementation is loosely based on code for 
	// "Differentiable Point-Based Radiance Fields for 
	// Efficient View Synthesis" by Zhang et al. (2022)
	glm::vec3 pos = means[idx];
	glm::vec3 dir = pos - glm::make_vec3(campos);
	...
	bool prefiltered)
{
	...
	{
		// compute cam position from view matrix
		float3 t = {-viewmatrix[12], -viewmatrix[13], -viewmatrix[14]};
		float3 cam_pos = transformVec4x3Transpose(t, viewmatrix);
		glm::vec3 result = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, (float*)&cam_pos, shs, clamped);
